...
const ShelfPositionRow = (props) => {
  const context = React.useContext(MyContext);
  const { row } = props;
  const [open, setOpen] = React.useState(false);
  
  const renderCategoryName = (type) => {
    let categoryName = context.types[type]?.name;
    return(categoryName);
  };
...
export default ShelfPositionRow;